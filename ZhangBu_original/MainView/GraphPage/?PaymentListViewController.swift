...
class PLViewController: UIViewController, UITableViewDelegate, UITableViewDataSource{
...
//    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        performSegue(withIdentifier: "toDitail", sender: indexPath.row)
        tableView.deselectRow(at: indexPath, animated: true)
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let detailVC = segue.destination as? AddPaymentVC {
            let indexPath = sender as! Int
            detailVC.current = (payments[indexPath],false)
        }
    }
}
